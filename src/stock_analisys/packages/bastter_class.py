...
# =============================================================================
    ...
    def company_data_extract(self):
        ...
        teste_company_info = self.driver.find_element_by_xpath('//*[@id="dados-empresa"]')
        
        # Se está carregado, passa pra frente, senão para. 
        if teste_company_info.is_displayed() == True:
            
            self.driver.find_element_by_xpath('//*[@id="dados-empresa"]/span[2]').click()
        ...
        # Se for uma STOCK 
            ...
            self.industry_group = soup.find('span', class_='ativo-industry-group').get_text()
            self.industry_category = soup.find('span', class_='ativo-industry-category').get_text() 
...
# =============================================================================
    ...
    def table_extract(self):
        ...
        # Clicando no item (com teste se ele existe)
        teste_selected_data = self.driver.find_element_by_xpath('//*[@id="quadro-simples-menu"]/span[2]') 
        
         
        # Se está carregado, passa pra frente, senão para. 
        if teste_selected_data.is_displayed() == True:
            
            self.driver.find_element_by_xpath('//*[@id="quadro-simples-menu"]/span[2]').click()
        ...
        simple_balance = soup.find("table",{"class":"evanual quadro table table-striped table-hover marcadagua"})
        ...
        self.df_list = self.df_list.applymap(lambda x: x.replace(".","").replace(",",".").replace('L','0').replace('-','-0'))
        ...
        # Nomeação do Header
        df_cols = ['Year', 'Net Revenue', 'Net Income', 'EPS', 'EBITDA', 'Net Debt', 'ND/EBITDA', 'FCF','FCF/Share']
...
# =============================================================================
    ...
    def reit_handling(self):
            ...
            reits_analised = pd.read_csv('data/avaliações_bastter/bastter_reits.csv')
            ...
            # Salva o DF
            df_merge.to_csv('data/avaliações_bastter/bastter_reits.csv', index=False)
            print('')
            self.print_lines()
            print('----------------------- Salvo para Avaliação Posterior--------------------------------- ')
            self.print_lines()
...
# =============================================================================
    ...
    def income_graph(self):
        x_cordinates = [self.df_list['Year'].iloc[0], self.df_list['Year'].iloc[-1]]
        y_cordinates = [0,0]
        ...
        plt.plot(self.df_list['Year'], self.df_list['Net Income'], label='lucro', color = 'green', marker='.', markersize = 10, linestyle='-' )

        plt.plot(x_cordinates, y_cordinates, label='zero', color = 'red', marker='.', markersize = 10, linestyle='--' )

        #Labels
        plt.title(f'Net Profit: {self.ticker} {self.company_name}', fontdict={'fontsize': 18}, color= 'black')
        plt.xlabel('Year', color = 'black')
        plt.ylabel('Net Profit (mil)', color = 'black')

        #Ticks
        plt.xticks(self.df_list['Year']) #Força o gráfico mostrar todos os anos
...
# ======================================================================================
    ...
    def income_percentual(self):
        ...
        for lucro_ano in self.df_list['Net Income']:
            ...
            percentual_mudanca = int((lucro_ano - lucro_ano_anterior)/(abs(lucro_ano_anterior + 1))*100)
...
# =====================================================================================
    ...
    def avalicao_stock(self):
        ...
        self.df_individual_stock = pd.DataFrame.from_dict(stock_info_dict, orient='index')
...
# =============================================================================
    ...
    def csv_storage(self):
        ...
        stocks_analisados = pd.read_csv('data/avaliações_bastter/bastter_stocks_analised.csv')
        ...
        print(f'Total de empresas analisadas: [{df_merge.shape[0]}]')
        self.print_lines()
        
        # Salva o DF com a empresa
        df_merge.to_csv('data/avaliações_bastter/bastter_stocks_analised.csv', index=False)
        
        # Salva os dados da empresa 
        self.df_list.to_csv(f'data/balancos_simplificados/{self.ticker} - {self.company_name} - Simple Balance.csv', index=False)
        self.print_lines
        print('Dados da Empresa Guardados com Sucesso')
